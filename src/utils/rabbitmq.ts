...
const defaultPublishingOptions = { persistent: true, mandatory: true };
...
export class RabbitMQExchange {
  ...
  private static activeConsumers: ActiveConsumers[] = [];
  private static activeBindings: QueueBinding[] = [];
  ...
  static async messageConsumer(
    ...
    const wrappedHandler = async (msg: ConsumeMessage | null) => {
      try {
        ...
        await Promise.resolve(handler(processedMsg, channel));
      } catch (error: any) {
  ...
  static async setupQueueBinding(bindings: QueueBinding[] = []) {
    try {
      const connection = await this.getConnection();
      const channel = await connection.createChannel();
